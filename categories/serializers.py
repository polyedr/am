...
class RecursiveField(Field):
    ...
    def __init__(self, to=None, **kwargs):
        ...
        super_kwargs = dict(
            (key, kwargs[key])
            for key in kwargs
            if key in inspect.signature(Field.__init__)
        )
...
class CategoryListSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        for child_data in children:
            s = CategoryListSerializer(
                data=child_data, context={"parent": category}, partial=True
            )
    ...
    def validate(self, data):
        # The children validation for the serializers.Serializer
        try:
            children = self.parent.initial_data.get("children")
            if children:
